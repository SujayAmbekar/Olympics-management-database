...
def display_staff():
    conn = psql.connect(host="localhost",database="olympics",user="tstaff",password="tstaff")
    cur = conn.cursor()
...
def sort():
    conn = psql.connect(host="localhost",database="olympics",user="tstaff",password="tstaff")
    cur = conn.cursor()
    if(currentTable == "display_staff"):
        cur.execute("select * from supportstaff order by name;")
        supportstaff(cur)
    if(currentTable == "display_facility"):
        cur.execute("select * from facility order by name;")
        facility(cur)    
    if(currentTable == "display_transport"):
        cur.execute("select * from transportation order by capacity;")
        transport(cur)      
...
def bus():
    conn = psql.connect(host="localhost",database="olympics",user="tstaff",password="tstaff")
    cur = conn.cursor()
    cur.execute("select * from transportation where type='Bus';")
    transport(cur)
    conn.commit()
    conn.close()
...
def car():
    conn = psql.connect(host="localhost",database="olympics",user="tstaff",password="tstaff")
    cur = conn.cursor()
    cur.execute("select * from transportation where type='Car';")
    transport(cur)
    conn.commit()
    conn.close()
...
def taxi():
    conn = psql.connect(host="localhost",database="olympics",user="tstaff",password="tstaff")
    cur = conn.cursor()
    cur.execute("select * from transportation where type='Taxi';")
    transport(cur)
    conn.commit()
    conn.close()
...
def tokyo():
    conn = psql.connect(host="localhost",database="olympics",user="tstaff",password="tstaff")
    cur = conn.cursor()
    cur.execute("select * from facility where location='Tokyo';")
    facility(cur)
    conn.commit()
    conn.close()
...
def hokaido():
    conn = psql.connect(host="localhost",database="olympics",user="tstaff",password="tstaff")
    cur = conn.cursor()
    cur.execute("select * from facility where location='Hokaido';")
    facility(cur)
    conn.commit()
    conn.close()
...
showstaff.place(relx=0.135,rely=0.2)
...
showfacility.place(relx=0.2,rely=0.2)
...
showtransport.place(relx=0.265,rely=0.2)
...
Bus_button = TkinterCustomButton(bg_color=None, fg_color="#212F3D", border_color="#117A65", hover_color="#34495E", text_font=None,
                            text="Bus", text_color="white", corner_radius=0, border_width=4, width=100, height=50, hover=True, command= bus)
Bus_button.place(relx=0.60,rely=0.60)

Car_button = TkinterCustomButton(bg_color=None, fg_color="#212F3D", border_color="#117A65", hover_color="#34495E", text_font=None,
                            text="Car", text_color="white", corner_radius=0, border_width=4, width=100, height=50, hover=True, command= car)
Car_button.place(relx=0.680,rely=0.60)

Taxi_button = TkinterCustomButton(bg_color=None, fg_color="#212F3D", border_color="#117A65", hover_color="#34495E", text_font=None,
                            text="Taxi", text_color="white", corner_radius=0, border_width=4, width=100, height=50, hover=True, command= taxi)
Taxi_button.place(relx=0.760,rely=0.60)

Tokyo = TkinterCustomButton(bg_color=None, fg_color="#212F3D", border_color="#117A65", hover_color="#34495E", text_font=None,
                            text="Tokyo", text_color="white", corner_radius=0, border_width=4, width=100, height=50, hover=True, command= tokyo)
Tokyo.place(relx=0.72,rely=0.30)

Hokaido = TkinterCustomButton(bg_color=None, fg_color="#212F3D", border_color="#117A65", hover_color="#34495E", text_font=None,
                            text="Hokaido", text_color="white", corner_radius=0, border_width=4, width=100, height=50, hover=True, command= hokaido)
Hokaido.place(relx=0.72,rely=0.40)
